...
public class SmsServiceTest {
	...
	private SmsService defaultSms;
	@Autowired
	@Qualifier("smsServiceImplmw")
	private SmsService mwSms;
	@Autowired
	...
	public void sendSmsDefaultTest() {
		...
		// 单挑发送
		defaultSms.sendSms("18704282857", "你好");
		System.out.println("\r\n");

		// 多条相同内容发送
		defaultSms.sendMultSms(new String[] { "18704282857", "18704282857", "18704282857" }, "你好");
		System.out.println("\r\n");

		// 多条不同内容发送
		defaultSms.sendMultSms(new String[] { "18704282857", "18704282857", "18704282857" }, new String[] { "大连", "北京", "上海" });
	}
	...
	public void sendSmsServiceImplIncake() {
		...
		// 单挑发送
		Map<String, SmsResponse> rx = incakeSms.sendSms("18704282857", str + "【单挑发送】");
		rx.forEach((key, value) -> System.out.println(key + " : " + value));

		// 多条相同内容发送
		rx = incakeSms.sendMultSms(new String[] { "18704282857", "1870428285712" }, str + "【多条相同内容发送】");
		rx.forEach((key, value) -> System.out.println(key + " : " + value));
		...
		rx = incakeSms.sendMultSms(new String[] { "18704282857", "18704282857" }, new String[] { str + "【多条不同内容发送】", str + "【多条不同内容发送】" });
		rx.forEach((key, value) -> System.out.println(key + " : " + value));
	}
