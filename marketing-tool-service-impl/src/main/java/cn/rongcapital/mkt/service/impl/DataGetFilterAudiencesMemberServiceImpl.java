...
@Service
public class DataGetFilterAudiencesMemberServiceImpl implements DataGetFilterAudiencesStrategyService{
	...
	public Integer getAudiencesCount(Map<String, Object> paramMap ){
		...
		return dataMemberDao.getAudiencesCount(paramMap);
	...
	public List<String> getAudiencesIds(Map<String, Object> paramMap){
		...
		return dataMemberDao.selectMappingKeyIds(paramMap);	
	}
	...
	private <D extends BaseDataFilterDao<?>> List<Integer> filterContactId(List<Integer> contactIdList) {
		...
		for (Integer contactId : contactIdList) {
			...
			if (columnName == null) {
				...
				List<String> columnNameList = dataMemberDao.selectColumns();
