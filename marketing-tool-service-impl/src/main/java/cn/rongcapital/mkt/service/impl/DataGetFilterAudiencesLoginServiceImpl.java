...
@Service
public class DataGetFilterAudiencesLoginServiceImpl implements DataGetFilterAudiencesStrategyService{
	...
	public <T extends BaseQuery> Map<String,Object> getData(Integer mdType,
		...
		List<DataLogin> dataList = new ArrayList<DataLogin>();
		
		Integer totalCount = 0;
		if (mdDataList != null && mdDataList.size() > 0) {
			
			dataList = dataLoginDao.selectByBatchId(paramMap);
			totalCount = dataLoginDao.selectCountByBatchId(paramMap);
		}
	...
	public Integer getAudiencesCount(Map<String, Object> paramMap ){
		...
		return dataLoginDao.getAudiencesCount(paramMap);
	...
	public List<String> getAudiencesIds(Map<String, Object> paramMap){
		...
		return dataLoginDao.selectMappingKeyIds(paramMap);	
	}
	...
	private <D extends BaseDataFilterDao<?>> List<Integer> filterContactId(List<Integer> contactIdList) {
		...
		for (Integer contactId : contactIdList) {
			...
			if (columnName == null) {
				...
				List<String> columnNameList = dataLoginDao.selectColumns();
