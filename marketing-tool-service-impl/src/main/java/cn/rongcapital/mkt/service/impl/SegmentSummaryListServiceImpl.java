...
@Service
public class SegmentSummaryListServiceImpl implements SegmentPublishstatusListService {
	...
	public SegmentPublishstatusListOut segmentPublishstatusList(String userToken, 
		...
		SegmentationHead head = constructSegmentationHead(publishStatus, index, size, keyword);
		int totalCount = segmentationHeadDao.selectListCount(head);
		List<SegmentationHead> reList = segmentationHeadDao.selectListByKeyword(head);
	...
	private SegmentationHead constructSegmentationHead(Integer publishStatus, Integer index, Integer size,
			String keyword) {
