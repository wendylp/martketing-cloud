...
public class MktDataApi {
    ...
    public Object getFilterAudiences(@Valid DataGetFilterAudiencesIn dataGetFilterAudiencesIn) {
        ...
        switch (dataGetFilterAudiencesIn.getContactWayList()) {
        case 1:
            contactWay = "mobile";
            break;
        case 2:
        ...
        String[] statictisTypeIds = new String[dataGetFilterAudiencesIn.getMdTypes().size()];
        for (int i = 0; i < statictisTypeIds.length; i++) {
            statictisTypeIds[i] = dataGetFilterAudiencesIn.getMdTypes().get(i).toString();
        }
    ...
    @CaasAuth(res = "#orgId", oper = "T(cn.rongcapital.mkt.common.constant.ApiConstant).CAAS_READ", type = CaasAuth.Type.SpEl)
    public BaseOutput audienceCount(@NotEmpty @QueryParam("user_token") String userToken, @NotNull @QueryParam("org_id") Integer orgId, @QueryParam("firsthand") Boolean firsthand) {
        return audienceListService.audienceCount(userToken, orgId, firsthand);
    }
