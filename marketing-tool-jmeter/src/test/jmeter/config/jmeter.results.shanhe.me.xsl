    ...
    <xsl:template match="/testResults">
        ...
        <head>
            ...
            <script type="text/javascript"><![CDATA[
                ...
                // 去除接口名前面的“接口:”
                var update_method = function() {
                    var methods = document.getElementsByName("methods");
                    var len = methods.length;
                    for( var i = 0; i < len; ++i ) {
                        var method = methods[i];
                        old_str=method.innerHTML;
                        new_str=old_str.replace("接口:","");
                        method.innerHTML=new_str;
                    }
                };

                // 批量格式json
                var update_json = function() {
                    var jsons = document.getElementsByName("jsons");
                    var len = jsons.length;
                    for( var i = 0; i < len; ++i ) {
                        var json = jsons[i];
                        var old_str=json.innerHTML;
                        new_str=formatJson(old_str);
                        json.innerHTML=new_str;
                    }
                };

                // 点击格式json
                var update_json_click = function(json) {
                    var old_str=json.innerHTML;
                    var reg = new RegExp("\n","g");
                    var len = old_str.match(reg);
                    if(len == null || len.length <= 0) {
                        new_str=formatJson(old_str);
                    } else {
                        new_str=old_str.replace(/\s+/g ,"");
                    }
                ...
                var update_url = function() {
                    ...
                    for( var i = 0; i < len; ++i ) {
                        var url = urls[i];
                        var old_str=url.innerHTML;
                        new_str=old_str.replace(/&/g,"\r\n&");
                        url.innerHTML=new_str;
                    }
                };

                // 点击格式url
                var update_url_click = function(url) {
                    var old_str=url.innerHTML;
                    var reg = new RegExp("\n","g");
                    var len = old_str.match(reg);
                    if(len == null || len.length <= 0) {
                    ...
                    <xsl:for-each select="*">
                        ...
                        <li onclick="return onclick_li(this);">
                            <div>
                                ...
                                <div class="group">接口</div>
                                <div class="zebra">
                                    <table>
                                        <tr><td class="data key">接口名称</td><td class="data delimiter">:</td><td class="data" name="methods"><xsl:value-of select="@lb"/></td></tr>
                                ...
                                <div class="zebra">
                                    <table>
                                        <tr><td class="data key">Method/Url</td><td class="data delimiter">:</td><td class="data"><pre class="data" name="urls" ><xsl:value-of select="method"/><xsl:text> </xsl:text><xsl:value-of select="java.net.URL"/></pre></td></tr>
                                        <tr><td class="data key">Query String</td><td class="data delimiter">:</td><td class="data" ><pre class="data" name="jsons"><xsl:value-of select="queryString"/></pre></td></tr>
                                ...
                                <div class="zebra">
                                    <table>
                                        ...
                                        <tr><td class="data key">Response Data</td><td class="data delimiter">:</td><td class="data" ><pre class="data" name="jsons" ><xsl:value-of select="responseData"/></pre></td></tr>
